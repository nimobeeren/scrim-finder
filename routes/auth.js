...
router.post("/anonRegister", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send("Could not generate authorization token: " + e.message || e);
  }
...
router.post("/refresh", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send("Could not verify authorization token: " + e.message || e);
  }
  if (!userId) {
    res.sendStatus(401);
    return;
  ...
  try {
    ...
    res
      .status(410)
      .send(
        "Anonymous user has expired. Please register a new user. " +
          e.message || e
      );
    return;
  }
    ...
    if (err || !result || !result.authenticated) {
      ...
      if (user && user._id) {
        ...
        try {
          ...
          res
            .status(500)
            .send("Could not create new Steam user: " + e.message || e);
          return;
        }
      }
